  ...
  var includes = config.includes
        ...
        .map(({ file, path }) => {
          ...
          renderer.code = function (code, language) {
            ...
            const isResponse = path.indexOf('response') >= 0;
          
            const lang = isResponse ? 'json': language;
          
            var highlighted = lang ? highlight.highlight(lang, code).value
                                       : highlight.highlightAuto(code).value;
          
            var className = `highlight ${lang} ${isResponse ? 'response' : ''}`;
            var preElm = '<pre class="' + className + '"><code>' + highlighted + '</code></pre>';

            var wrapperClassName = `code-wrapper ${lang} ${isResponse ? 'response-wrapper' : ''}`;

            var copyElm = '<i class="fa fa-copy"></i>';

            var codeElm = '<span class="' + wrapperClassName + '">' + preElm + copyElm + '</span>';

            return codeElm;
          };

          return marked(file, { renderer: renderer });
        });
